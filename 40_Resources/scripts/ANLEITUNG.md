...
```
**Kein Client-Secret, kein Server.** Login per Geräte-Code (MSAL); der Token wird **verschlüsselt im OS-Keystore** gecacht — **macOS Keychain** bzw. **Windows DPAPI** (via `msal_extensions`), **nie** als Klartext-Datei. Details: Abschnitt „🔒 Sicherheit & .gitignore".
...
> [!danger] Eiserne Regel: Tokens & Secrets gehören in den OS-Keystore — NIE als Klartext
> Jeder MSAL-Token-Cache wird **verschlüsselt** abgelegt: **macOS Keychain** bzw. **Windows DPAPI**, beides über `msal_extensions` (`PersistedTokenCache` + `KeychainPersistence` / `build_encrypted_persistence()`). **Vorlage:** `auth_common.py` → `build_cache()` (M365) und `build_pbi_cache(tenant)` (Power BI). **Verboten:** `SerializableTokenCache`, das eine Klartext-`.bin` schreibt — die landet sonst über Dropbox/Git in der Cloud. **„Nur lokal" ist NICHT sicher,** solange der Ordner Dropbox-synchronisiert ist.

Diese Dateien bleiben **lokal** und gehören trotzdem in die `.gitignore` (Mail-/Chat-Inhalte bzw. evtl. Alt-Caches):
```
40_Resources/scripts/.token_cache.bin         # Alt-Format; neu: Keychain (s. o.)
40_Resources/scripts/.pbi_token_cache_*.bin   # Power-BI Alt-Caches
*.bin
...
**/__pycache__/
```
- Nur **`…​.Read`** (+ `Mail.ReadWrite` für Entwürfe) → kein Senden/Löschen.
- Du liest **nur dein eigenes** Postfach/Chats.
- **NICHT** in den Vault legen: **Tokens, Secrets**, Passwörter, Bank-/Gesundheitsdaten.
- **Bei Leck:** in Keychain migrieren → Klartext löschen → aus Git-Historie purgen → **Token widerrufen** (wichtigster Schritt).
...
- **`com.miragliabi.m365-weekly.plist`** = launchd-Job (**freitags 23:40**). Installieren:
  ```bash
  ...
  ```
- **`weekly_watchdog.sh`** + **`com.miragliabi.m365-weekly-watchdog.plist`** = Erinnerungs-Watchdog: läuft täglich 13:00 + bei jedem Login und prüft, ob der letzte Lauf (jüngstes `weekly_*.log`) älter als 7,5 Tage ist (z. B. Laptop war Fr 23:40 aus). Falls überfällig → **macOS-Notification + Flag-Notiz `00_Inbox/⚠️ M365 Wochen-Refresh ueberfaellig.md`** mit Hinweis, `weekly_refresh.sh` manuell zu starten. Die Erinnerung **löscht sich selbst**, sobald wieder gelaufen wurde. Installieren:
  ```bash
  cp com.miragliabi.m365-weekly-watchdog.plist ~/Library/LaunchAgents/
  launchctl load -w ~/Library/LaunchAgents/com.miragliabi.m365-weekly-watchdog.plist
  ```
...
> - **Claude Desktop ↔ Obsidian-MCP (Mac-Host & Windows/Parallels):** [[40_Resources/Obsidian-MCP für Claude Desktop (Mac-Host & Windows-Parallels)]] — Fix für „verliert nach Stunden die Verbindung"
