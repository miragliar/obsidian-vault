...
# Triage-Tags: bereits eingeordnete Nicht-Kunden (Privat/Dienstleister/Intern/
# Sammelpostfach/zu prüfen) — werden aus den „zu verknüpfen"/„Firmen-Kandidaten"-
# Listen ausgeblendet, damit sie nicht jede Woche neu auftauchen.
TRIAGE_LINK = {"privat", "dienstleister", "intern", "mailbox", "review"}
TRIAGE_CHAR = {"privat", "intern", "mailbox", "review"}
...
def main():
    ...
    for n in PEOPLE.glob("*.md"):
        ...
        if not client and not (tags & TRIAGE_LINK):
        ...
        if not has_char and not (tags & TRIAGE_CHAR):
