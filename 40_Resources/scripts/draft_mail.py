...
def file_attachment(path):
    ...
    return {"@odata.type": "#microsoft.graph.fileAttachment", "name": p.name,
            "contentType": ctype, "contentBytes": b}
...
def create_draft(token, to, cc, subject, body, body_type="HTML", reply_to=None, signature=True, attachments=None):
    if signature:
    ...
    if reply_to:
        ...
        r.raise_for_status()
        did = r.json()["id"]
        # bestehenden zitierten Verlauf holen und eigenen Text DAVOR setzen → Reply-Historie bleibt erhalten
        cur = requests.get(f"{GRAPH}/me/messages/{did}", headers=h, params={"$select": "body"}, timeout=30)
        quoted = (cur.json().get("body") or {}).get("content", "") if cur.ok else ""
        patch = {"body": {"contentType": body_type, "content": (body or "") + quoted}}
        if to:
        ...
        for att in (attachments or []):
            raf = requests.post(f"{GRAPH}/me/messages/{did}/attachments", headers=h,
                                json=file_attachment(att), timeout=60)
            raf.raise_for_status()
        return r2.json()
    ...
    if needs_logo:
        ...
        ra.raise_for_status()
    for att in (attachments or []):
        raf = requests.post(f"{GRAPH}/me/messages/{created['id']}/attachments", headers=h,
                            json=file_attachment(att), timeout=60)
        raf.raise_for_status()
    return created
...
def main():
    ...
    ap.add_argument("--attach", help="Datei-Anhänge, komma-getrennt")
    ...
    if args.file:
        ...
        attachments = att.split(",") if isinstance(att, str) else att
    else:
        ...
        attachments = (args.attach or "").split(",") if args.attach else []

    token = get_token()
    draft = create_draft(token, to, cc, subject, body, body_type, reply_to, signature, attachments)
