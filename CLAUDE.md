...
### Journal-Einträge (Dataverse, schreibend mit Gate) — VORHANDEN
Skript `40_Resources/scripts/journal_create.py` legt Journal-Einträge in der Accounting-App an (gleiche Org `org62e5ae4f`, gleicher Keychain-Token). **Default ist Dry-Run (schreibt NICHTS); erst `--commit` legt an.** Eintrag braucht nur: **Firma · Kontakt (Pflicht)** · [Leistungspaket, falls die Firma welche hat] · **Beschreibung** (sachlich/kurz, 1–3 Sätze) · **Stunden** (nur verrechenbare; Gratis-Anteile inline „(2h Kostenlos)"). **IMMER zuerst Dry-Run zeigen, `--commit` erst nach Raouls Freigabe.** Bei 0 oder >1 Treffern (Firma/Kontakt/LP): **Kandidaten zeigen und fragen — NIE selbst „das plausibelste" wählen oder Namen still korrigieren.** Beträge/Stundensatz/J-Nummer rechnet die App serverseitig — nie mitsenden. Duplikatschutz (Firma+Datum+Stunden). Geschäftsregeln `LP_PFLICHT_FIRMEN`/`GESPERRTE_FIRMEN` (z. B. `MVM AG` gesperrt → `MVM Services AG`) stehen im Skriptkopf. Routing read-only prüfen: `journal_create.py --suggest "<Kunde>"`.
Überwachung: `journal_audit.py` (read-only, DE-Kunden ohne MwSt + LP-Pflicht; Alarm-Notiz landet in `_imports/` bzw. per `JOURNAL_AUDIT_INBOX` überschreibbar). Setup-Anleitung: [[40_Resources/scripts/_journal_incoming/Journal-Setup — Obsidian + Claude (Team-Anleitung).md]] (von Giovanni, 2026-07-02).
