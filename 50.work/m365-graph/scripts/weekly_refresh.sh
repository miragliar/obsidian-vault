...
# --- Konfiguration ---
# 1) .env laden falls vorhanden (vault-spezifische Pfade, z. B. CLIENTS_DIR=26_Firmen).
if [ -f .env ]; then
    set -a; source .env; set +a
fi
# 2) Fallback-Defaults (entsprechen Michaels PARA-Layout + Giovannis App-IDs):
...
: "${INBOX_DIR:=00_Inbox}"
: "${PEOPLE_DIR:=25_People}"
export M365_CLIENT_ID M365_TENANT_ID CLIENTS_DIR INBOX_DIR PEOPLE_DIR
